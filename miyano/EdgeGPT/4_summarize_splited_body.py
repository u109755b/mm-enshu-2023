...
async def main():
    ...
    # すでに要約済みの場合, 実行しない
    if os.path.exists(f"log/{title}/{scene_group_name}/summary.txt"):
    ...
    # 要約結果を"log/{title}/{scene_group_name}/summary.txt"にまとめる
    with open(f"log/{title}/{scene_group_name}/summary.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(summaries))
