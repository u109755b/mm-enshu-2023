...
# TODO: プロンプトの改良
def create_knowledge_graph(storyID, show_log):
    # 分割結果の読み込み
    ...
    for sceneID in range(scene_num):
        with open(f"log/{storyID}/body_scene{sceneID}.txt", encoding="utf-8") as f:
            scene_bodys.append(f.read())

    # トークン数を記録
    prompt_tokens, completion_tokens = 0, 0

    # node, edge の抽出
    for sceneID, scene_body in enumerate(scene_bodys):
        # node 抽出用プロンプトの作成
        node_user_message = (
...
def main():
    ...
    # すでに実行済みの場合, 実行しない
    if os.path.exists(f"log/{storyID}/node_scene0.txt"):
    ...
    create_knowledge_graph(storyID, show_log)
