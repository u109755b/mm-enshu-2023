...
def add_novel_to_DB_by_title(conn, cur, title, info: dict):
    ...
    if row is not None:
        ...
        cur.execute(
            """
            INSERT INTO novel (title, author, genre, gutenberg_id)
            VALUES (%s, %s, %s, %s) RETURNING id
        """,
            (title, author, genre, info["GutenbergID"]),
        )
...
def search_novel_by_title(conn, cur, title_vector, top_k):
    title_vector_list = (
    ...
    cur.execute(
        ...
        ORDER BY novel_title_vector.title_vector <-> CAST(%s AS vector) LIMIT %s;
        """,
        (title_vector_list, top_k),
    )
    results = cur.fetchall()
    return results


def search_novel_by_id(conn, cur, id_list):
    cur.execute(
        """
    SELECT novel.*
    FROM novel
    WHERE novel.gutenberg_id = ANY(%s);
    """,
        (id_list,),
    )
    results = cur.fetchall()
...
def add_title_to_vector_talbe(conn, cur, novel_id, title_vector):
    title_vector_list = (
        title_vector.tolist() if isinstance(title_vector, np.ndarray) else title_vector
    )
    cur.execute(
