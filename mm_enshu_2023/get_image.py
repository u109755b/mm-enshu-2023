...
def save_image(image_id, novel_name, character_name):
    """画像データをファイルに保存する"""
    save_directory = f"../images/{novel_name}"
    os.makedirs(save_directory, exist_ok=True)
    save_path = os.path.join(save_directory, f"{character_name}.png")

    with open(save_path, 'wb') as image_file:
        image_file.write(openai.Image.download(image_id=image_id).content)
...
def fetch_and_save_dalle_image_with_logging(novel_name, character_name, api_key_file_path, image_quality, log_file_path):
    ...
    response = send_dalle_request(api_key, prompt, image_quality)
